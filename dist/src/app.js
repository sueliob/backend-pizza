...
app.post('/api/admin/refresh', async (c) => {
    ...
    if (!cookies['refresh_token'])
        return c.json({ error: 'No refresh' }, 401);
    if (!csrfHeader || !csrfCookie || csrfHeader !== csrfCookie)
        return c.json({ error: 'CSRF' }, 403);
    const incoming = cookies['refresh_token'];
    const hash = await sha256Hex(incoming);
    const now = new Date();
    const [sess] = await db.select().from(sessions).where(and(eq(sessions.refreshHash, hash), isNull(sessions.revokedAt), gt(sessions.expiresAt, now))).limit(1);
    if (!sess)
...
app.get('/api/admin/dashboard', async (c) => {
    ...
    try {
        ...
        const monthlyRevenue = allOrders.reduce((sum, o) => sum + parseFloat(o.total || '0'), 0);
